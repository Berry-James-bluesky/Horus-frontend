...
const Sidebar: React.FC = () => {
  ...
  const burgerIconStatus = showLinks
    ? "transform rotate-90"
    : "transform rotate-0";
  ...
  return (
    <div
      className={`h-screen bg-gray-50 border-r-2 background-black pl-6 sticky fixed top-0 left-0 w-80 z-10 overflow-hidden sidebar ${size}`}
    >
      <div className="pt-4 sidebar-toggle-container">
        <button
          onClick={() => {
            size === "full-size-sidebar"
              ? setSize("small-size-sidebar")
              : setSize("full-size-sidebar");
          }}
      ...
      <div className="flex align-center w-full row-span-1 pt-6 pb-2 mb-6 branding-container">
        ...
        <button
          className={`hamburger-icon md:hidden block transition duration-300 ${burgerIconStatus}`}
          onClick={handleLinksClick}
        >
          <Icon name={"bars"} />
        </button>
        <span className={"font-sans ml-4 text-gray-200 md:hidden block"}>
          {currentLocation}
        </span>
      </div>
      <div className={`grid grid-row-6 navlink-container ${linksStatus}`}>
        <div
          className={`block md:hidden sidebar-header flex items-center justify-start pl-4`}
          style={{ backgroundImage: `url(${Pyramids})` }}
        >
          <h2 className={"font-serif font-normal"}>Horus</h2>
        </div>
        ...
        <SidebarItem
          ...
          label="Calendar"
        />
        <SidebarItem link="/clients" iconName="briefcase" label="Clients" />
      ...
      <div>
        <Profile
          userName="Test name"
          userImg="https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/luqnf2opx6fub6apceix.jpg"
        />
      </div>
    </div>
  );
